...
{ 
    ...
    for ($row_start; $row_start <= $highestRow1; $row_start++) //$highestRow
    {
          ...
          if(!empty($oracle_no) && !empty($name) && !empty($loan_amount) && !empty($expected_monthly_repayment_amount) )
          {
       ...
       $result3 = mysql_query("SELECT * from expected_repayment where oracle_number='$oracle_no' and expected_monthly_repayment_month='$repayment_month' and expected_monthly_repayment_year='$repayment_year' and microfinance_bank='$mfb'");
          ...
          {
              echo '<center><strong>Error!</strong><span class="text-danger">ORACLE NO , NAME,  LOAN AMOUNT,  EXPECTED_MONTHLY_REPAYMENT are mandatory</span></center>';
              $error_report=1 ;
          }
    }//end for
    if($error_report==1)
	...
	{
		$this->load->model('main_model');
		$data['expected'] = $this->main_model->get_expected_record();
	...
	{
		$this->load->model('main_model');
		$data['expected'] = $this->main_model->get_merged_result();
	...
	function search_merged()
	{
		$this->load->model('main_model');
		$btn_search = $this->input->post('btn_search');
		if($btn_search == "Search")
		{
			$data['expected'] = $this->main_model->search_merged();
			$data['title'] = 'Merged Result';
			$this->load->view('header/header',$data);
			$this->load->view('content/view_merged_result',$data);
			$this->load->view('footer/footer');
		...
		else if($btn_search == "Spool")
		{
           ...
           $report=$this->main_model->spool_merged();
	...
	{
		$this->load->model('main_model');
		$data['cumm'] = $this->main_model->get_cummulative_report();
		$data['title'] = 'Cummulative Report';
		$this->load->view('header/header',$data);
		$this->load->view('content/view_cummulative_report',$data);
		$this->load->view('footer/footer');
	}
 ...
 function insert_from_uploaded_actual($path)
 {
  ...
  { 
    ...
    for ($row_start; $row_start <= $highestRow1; $row_start++) //$highestRow
    {
          ...
          $name=$objWorksheet1->getCellByColumnAndRow(1, $row1)->getValue()?$objWorksheet1->getCellByColumnAndRow(1, $row1)->getValue():'';
		  $ministry=$objWorksheet1->getCellByColumnAndRow(2, $row1)->getValue()? $objWorksheet1->getCellByColumnAndRow(2, $row1)->getValue():'';
          $actual_repayment_amount=$objWorksheet1->getCellByColumnAndRow(2, $row1)->getValue()? $objWorksheet1->getCellByColumnAndRow(3, $row1)->getValue():'';
          /*$expected_monthly_repayment_amount=$objWorksheet1->getCellByColumnAndRow(3, $row1)->getValue()? $objWorksheet1->getCellByColumnAndRow(4, $row1)->getValue():'';
          $tenure=$objWorksheet1->getCellByColumnAndRow(4, $row1)->getValue() ? $objWorksheet1->getCellByColumnAndRow(5, $row1)->getValue() : ''; 
          $repayment_frequency=$objWorksheet1->getCellByColumnAndRow(5, $row1)->getValue() ? $objWorksheet1->getCellByColumnAndRow(6, $row1)->getValue() : '';*/
		...
		foreach($query->result() as $key)
		{
			...
			if($config == "equal")
			{
				...
				if($num_inserts=="1")
				{
					...
					$sql_c = "select * from cummulative_report where mfb_bank='$microfinance_bank' and month='$expected_monthly_repayment_month' and year='$expected_monthly_repayment_year'";
					$query = $this->db->query($sql_c);
					$no_rows = $query->num_rows();
					if($no_rows>0)
					{
						$sql_c_ = "update cummulative_report set cumm_deduction = cumm_deduction +'$amount_deducted',cumm_outstanding = cumm_outstanding +'$outstanding',cumm_remittance = cumm_remittance +'$remittance' where mfb_bank='$microfinance_bank' and month='$expected_monthly_repayment_month' and year='$expected_monthly_repayment_year'";
						$this->db->query($sql_c_);
					}
					else
					{
						$data_cumm =	array(
						'mfb_bank'=>$microfinance_bank,
						'month'=>$expected_monthly_repayment_month,
						'year'=>$expected_monthly_repayment_year,
